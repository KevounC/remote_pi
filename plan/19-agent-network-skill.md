...
The transport layer (`peer.request()`) blocks until the reply arrives
...
proceed. The transport supports this natively — every `peer.request()`
...
const [beAnswer, feAnswer] = await Promise.all([
  peer.request("backend", { question: "JWT shape?" }),
  peer.request("frontend", { question: "current theme tokens?" }),
]);
...
const answers = await Promise.all(
  peers.map((p) => peer.request(p, { question: "ETA for Y?" }))
);
...
const winner = await Promise.race([
  peer.request("worker-1", taskBody),
  peer.request("worker-2", taskBody),
]);
...
const settled = await Promise.allSettled([
  peer.request("a", q1, 30_000),
  peer.request("b", q2, 30_000),
]);
...
A: Technically yes (`to: <your own name>`), the broker delivers it
back. But useless — just do what you were going to do without the
message.
